"""
This script is used to filter grasp poses in acronym.
"""
...
sys.path.append('./')
from tools.utils import get_RT_from_grasp_params, R2q

# parse arguments
args = gymutil.parse_arguments(
    description="Template Grasp Filter",
    custom_parameters=[
        ...
        {"name": "--temp_file", "action": 'store_true', "default": False},
        {"name": "--old", "action": 'store_true', "default": False},
...
resolution = {'mug':300000, 'bottle':8000000, 'bowl': 3000000}
RESOLUTION_SECOND = 60

def batch_panda_sim(cate, filename, mode, scale):
    ...
    obj_asset_file = 'urdf/shapenet/{0}/{1}/{2}.urdf'.format(cate, mode, filename.replace('_old', ''))
    ...
    while time.time() - start_time < 1000:
        ...
        if dt_cnt > (43 * 60) and compute_finished==False:

            obj_pos = rb_states[obj_idxs, :3].cpu().numpy()
            success_num = np.sum(np.linalg.norm(obj_pos - init_obj_pos, axis=1) < 0.1)

            with open(os.path.join(args.pkl_root, 'train', filename + '.pkl'), 'rb') as f:
                grasp_info = pickle.load(f)
            success_index = np.where(np.linalg.norm(obj_pos - init_obj_pos, axis=1) < 0.1)
            grasp_info['grasp_params'] = np.array(grasp_info['grasp_params'])[success_index]
            print('left grasp nums: ', len(grasp_info['grasp_params']))
            with open(os.path.join(args.pkl_root, 'train', filename + '.pkl'), 'wb') as f:
    ...
    return 100 * success_num / num_envs


if __name__ == '__main__':
    mode = 'train'
    source_files_dict = {
        'mug':{'filename':'62634df2ad8f19b87d1b7935311a2ed0/0', 'scale':0.21342577},
        'bowl':{'filename':'8bb057d18e2fcc4779368d1198f406e7/0', 'scale':0.200506648},
        'bottle':{'filename':'3108a736282eec1bc58e834f0b160845/0', 'scale':0.201867973},
    }
    file = source_files_dict[args.category]['filename']
    scale = source_files_dict[args.category]['scale']

    file = file.replace('.pkl', '')
    print(file)
    sr = batch_panda_sim(args.category, file, mode, scale)