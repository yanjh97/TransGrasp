...
def get_mesh_color(mesh_points):

    mesh_colors = np.zeros_like(mesh_points)

    mesh_colors = np.clip(mesh_points/2+0.5,0,1) # normalize color to 0-1
...
def convert_sdf_samples_with_color_to_ply(
    ...
    mesh_colors = get_mesh_color(mesh_points)
...
def create_mesh(model, filename, N=128, max_batch=64 ** 3, offset=None, scale=None,level=0.0, get_color=True):
    start = time.time()
    ...
    while head < num_samples:
        ...
        input = {'coords':sample_subset}
        samples[head : min(head + max_batch, num_samples), 3] = (
            model(input)['model_out']
            .squeeze()#.squeeze(1)
            .detach()
            .cpu()
        )
...
p.add_argument('--logging_root', type=str, default='DIF_decoder/recon', help='root for logging')
p.add_argument('--config', default='DIF_decoder/configs/generate/mug.yml', help='generation configuration')
...
model = model.template_field
model.cuda()

# create save path
root_path = os.path.join(opt.logging_root, meta_params['experiment_name'], 'train')
utils.cond_mkdir(root_path)

# generate template meshes with color-coded coordinates
create_mesh(model, './DIF_decoder/recon/{0}/train/template'.format(meta_params['experiment_name']), N=128,level=opt.level)
