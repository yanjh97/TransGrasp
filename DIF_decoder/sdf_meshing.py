...
def create_mesh(model, filename, subject_idx=0, embedding=None, N=128, max_batch=64 ** 3, offset=None, \
               scale=None,level=0.0, get_color=True, nocs=False):
    start = time.time()
    ...
    if not get_color:
        ...
        return convert_sdf_samples_with_color_to_ply(
            model,
            embedding,
            sdf_values.data.cpu(),
            voxel_origin,
...
def get_nocs_color(mesh_points,embedding,model):
    model.eval()
...
def get_mesh_color(mesh_points,embedding,model):
    ...
    mesh_colors = np.clip(mesh_colors/2+0.6,0,1) # normalize color to 0-1
...
def get_mesh_points(
    ...
    verts = np.zeros((0, 3))
    try:
        verts, _, _, _ = skimage.measure.marching_cubes(
            numpy_3d_sdf_tensor, level=level, spacing=[voxel_size] * 3
        )
    except:
        pass
...
def convert_sdf_samples_with_color_to_ply(
    ...
    if nocs:
        mesh_colors = get_nocs_color(mesh_points,embedding,model)
    else:
